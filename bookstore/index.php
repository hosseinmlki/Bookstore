...
if(isset($_SESSION['id'])){
	...
	echo '<form class="hf" action="edituser.php"><input class="hi" type="submit" name="submitButton" value="ویرایش پروفایل"></form>';
	echo '</blockquote>';
	echo '</header>';
}
...
echo '<blockquote>';
    ...
    while($row = $result->fetch_assoc()) {
	    echo "<td>";
	    echo "<table>";
	   	echo '<tr><td>'.'<img src="'.$row["Image"].'"width="80%">'.'</td></tr><tr><td style="padding: 5px;">عنوان: '.$row["BookTitle"].'</td></tr><tr><td style="padding: 5px;">نویسنده: '.$row["Author"].'</td></tr><tr><td style="padding: 5px;">قیمت: تومان '.$row["Price"].'</td></tr><tr><td style="padding: 5px;">
	   	<form action="" method="post">
	   	تعداد: <input type="number" value="1" name="quantity" style="width: 20%"/><br>
	   	<input type="hidden" value="'.$row['BookID'].'" name="ac"/>
	   	<input class="button" type="submit" value="افزودن به سبد خرید"/>
	   	</form></td></tr>';
	   	echo "</table>";
	   	echo "</td>";
    }
    ...
    echo "<th style='text-align:left;'><i class='fa fa-shopping-cart' style='font-size:24px'></i> سبد خرید <form style='float:right;' action='' method='post'><input type='hidden' name='delc'/><input class='cbtn' type='submit' value='لغو خرید'></form></th>";
    $total = 0;
    while($row = $result->fetch_assoc()){
    	echo "<tr><td>";
    ...
    echo "جمع خرید: <b>تومان".$total."</b><center><form action='checkout.php' method='post'><input class='button' type='submit' name='checkout' value='پرداخت'></form></center>";
    echo "</td></tr>";
	echo "</table>";
	echo '</blockquote>';
?>
